...
# Add project root to path (go up 1 level: slack_worker -> root)
...
def format_release_message(releases, period):
    """Format releases into a readable Slack message"""
    message = f"*{period} Releases:*\n"
...
def preview_and_send():
    ...
    print(f"⚠️  Today is {days[day_of_week]} ({today.strftime('%b %d')})")
