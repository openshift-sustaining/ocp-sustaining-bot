    ...
    try:
        ...
        if os_name and os_name.strip().lower() == "linux":
            ...
            if "error" in server_status_dict:
                ...
                say(f":x: *EC2 instance creation failed.*\n> Error: {error_msg}")
                return
...
@command_meta(
    name="rota",
    description="Manage releases rotation assignments in Google Sheets(ROTA)",
    arguments={
        "action": {
        ...
        "pm": {
            "description": "Pach Manager username",
            "required": False,
            "type": "str",
        },
        "qe1": {
        ...
        "time": {
            "description": "Time period (This Week or Next Week)",
            "required": False,
            "type": "str",
        },
        "column": {
            "description": "Column to replace (pm, qe1, qe2)",
            "required": False,
            "type": "str",
        },
        "user": {
            "description": "New user for replacement",
            "required": False,
            "type": "str",
        },
    },
    examples=[
        "rota --add --release=4.15.1 --start=2024-01-08 --end=2024-01-12 --pm=@john --qe1=@jane --qe2=@paul",
        "rota --check --time='This Week'",
        "rota --check --release=4.15.1",
        "rota --replace --release=4.15.1 --column=pm --user=@new_person",
    ],
)
def handle_rota(say, user, params_dict):
    """
    ROTA command handler for the main bot.
    
    Commands:
    - add: Add a new release to the ROTA (admin only)
    - check: Check release details by version or time period
    - replace: Replace a team member for a release
    """
    # Check if gsheet is initialized
    if not gsheet:
        say("⚠️ ROTA system is not configured. Please contact an administrator.")
        logger.error("GSheet is not initialized. Check ROTA_SERVICE_ACCOUNT configuration.")
        return
    ...
    if params_dict.get("add"):
        ...
        if not rel_ver:
            say("Please provide a release version.")
            return
        ...
        say("Success! Release added to ROTA.")
        return
    ...
    # Replace team member , again, only ADMiNS can do this
        ...
        new_user = _get_name_from_userid(params_dict.get("user"))

        if not all([rel_ver, column]):
            say("Please provide `release` and `column` (eg. --column=qe1, --column=qe2).")
            return

        try:
            gsheet.replace_user_for_release(rel_ver, column, new_user)
        except ValueError as e:
            say(str(e))
            return

        say("Success! Team member replaced.")
        return
...
def _helper_format_rota_output(data: list) -> str:
    """Format ROTA data for display in Slack."""
...
def _helper_date_validation(date: str, day: int) -> str:
    """Validate that a date is in correct format and on correct day of week."""
